...
func TestAuthMiddleware(t *testing.T) {
	// structured as a table-driven test
	testCases := []struct {
		name          string
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// creates a new instance of an HTTP response recorder.
			recorder := httptest.NewRecorder()
			request, err := http.NewRequest(http.MethodGet, authPath, nil)
			require.NoError(t, err)

			tc.setupAuth(t, request, server.tokenMaker)
			// simulating an HTTP request to your server's router (likely an instance of the gin router)
			// and capturing the response using the recorder.
